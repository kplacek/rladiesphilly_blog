...
Using tidycensus data, we read in spatial data from tidycensus using the [rgdal](https://cran.r-project.org/web/packages/rgdal/index.html) package and plotted it using default settings:
...
Importantly, in order for two datasets to be joined together, they must have the same CRS! In our data, this is the case, but if you needed to change the CRS of your datasets, you can do so using the following command:
```{r include = F, eval = T}
