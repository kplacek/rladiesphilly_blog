...
Using tidycensus data, we read in spatial data from tidycensus using the rgdal package and plotted it using base R:
...
print("Coordinate Reference System for ACS Data")
acs_philly@proj4string

# The Philadelphia county boundary
print("Coordinate Reference System for Philadelphia County Data")
Phila_County@proj4string
```

Importantly, in order for two datasets to be joined together, they must have the same CRF! In our data, this is the case, but if you needed to change the CRF of your datasets, you can do so using the following command:
```{r include = F, eval = T}
...
```
Here, we have put both the Phila_County data from tidycensus and the ACS spatial data in the 'WGS84' CRF which is commonly used in GIS.

And then, you can join the two datasets together:
```{r}
acs_philly <-
...
Here, we plot the total number of married couples for each census tract in the Philadelphia area:
```{r include = T, eval = T}
tm_shape(acs_philly) + tm_fill("MedInc_Est", palette = "Reds")
```

We can further alter the presentation of this data by using built-in features from the tmap package:
```{r}
tm_shape(acs_philly) + 
